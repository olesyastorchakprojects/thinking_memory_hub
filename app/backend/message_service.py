...
class MessageService:
    ...
    async def process_message(self, request: MessageRequest) -> MessageResponse:
        try:
            classification = await self._intent_classifier.classify(request.text)
            state = await self._invoke_graph(request.text, classification.intent)
            return MessageResponse.model_validate(state["final_response"])
    ...
    async def _invoke_graph(self, user_message: str, intent: BackendIntent) -> dict:
        if intent == BackendIntent.save_note:
            save_input = await self._input_extractor.extract_save_input(user_message)
            return await self._save_graph.ainvoke(
                {"save_input": save_input.model_dump(mode="json")}
            )

        if intent == BackendIntent.search_notes:
            search_input = await self._input_extractor.extract_search_notes_input(user_message)
        ...
        if intent == BackendIntent.search_tags:
            search_input = await self._input_extractor.extract_search_tags_input(user_message)
            return await self._search_tags_graph.ainvoke(
                {"search_input": search_input.model_dump(mode="json", by_alias=True)}
            )

        if intent == BackendIntent.update_note:
            update_input = await self._input_extractor.extract_update_note_input(user_message)
